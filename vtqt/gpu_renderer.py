...
class CursorStyle(Enum):
    ...
    BLOCK = "block"          # Solid block (like default terminal)
    BAR = "bar"              # Thin vertical bar (like modern editors)
...
class GridRenderer:
    ...
    def render(self, cell_data: np.ndarray, viewport_size: Tuple[int, int],
               ...
               cursor_style: CursorStyle = CursorStyle.BLOCK,
               cursor_color: Tuple[float, float, float] = (0.8, 0.8, 0.8)):
        """
        Render cell grid with optional cursor.

        Args:
            cell_data: numpy array (rows, cols, 8)
                       [char_code, fg_r, fg_g, fg_b, bg_r, bg_g, bg_b, attrs]
            viewport_size: (width, height) in pixels
            cursor_pos: (row, col) cursor position, or None
            cursor_visible: whether cursor should be drawn (for blink)
            cursor_style: block, bar, or underline
            cursor_color: RGB tuple for cursor color
        """
        ...
        if cursor_visible and cursor_pos is not None:
            ...
            if 0 <= cursor_row < rows and 0 <= cursor_col < cols:
                self._render_cursor(
    ...
    def _render_cursor(self, row: int, col: int,
                       style: CursorStyle,
                       color: Tuple[float, float, float],
                       cell_data: np.ndarray):
        """
        Render cursor at specified position.

        Args:
            row, col: cursor position in grid
            style: cursor appearance style
            color: RGB tuple for cursor
            cell_data: cell array (for getting char under cursor)
        """
        x1 = col * self.cell_width
        y1 = row * self.cell_height
        x2 = x1 + self.cell_width
        ...
        if style == CursorStyle.BLOCK:
            # Filled block - draw with alpha for visibility
            glEnable(GL_BLEND)
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
            glDisable(GL_TEXTURE_2D)

            glBegin(GL_QUADS)
            glColor4f(cr, cg, cb, 0.7)  # Semi-transparent block
            glVertex2f(x1, y1)
            glVertex2f(x2, y1)
            glVertex2f(x2, y2)
            glVertex2f(x1, y2)
            ...
            # Also render the character under cursor in contrasting color
            # so it's visible through the block
            cell = cell_data[row, col]
            char_code = int(cell[0])

            if char_code > 32 and self.atlas:
                glEnable(GL_TEXTURE_2D)
                glBindTexture(GL_TEXTURE_2D, self.atlas.texture_id)

                u1, v1, u2, v2 = self.atlas.get_uv(char_code)

                # Get background color and use it for the text
                # (inverted effect)
                bg_r, bg_g, bg_b = cell[4], cell[5], cell[6]
        ...
        elif style == CursorStyle.BAR:
            # Thin vertical bar at left edge
            bar_width = max(2, self.cell_width // 8)
            ...
            glColor3f(cr, cg, cb)
            glVertex2f(x1, y1)
            glVertex2f(x1 + bar_width, y1)
            glVertex2f(x1 + bar_width, y2)
            glVertex2f(x1, y2)
            glEnd()

        elif style == CursorStyle.UNDERLINE:
            # Underline at bottom
            underline_height = max(2, self.cell_height // 8)
            ...
            glBegin(GL_QUADS)
            glColor3f(cr, cg, cb)
            glVertex2f(x1, y2 - underline_height)
            glVertex2f(x2, y2 - underline_height)
            glVertex2f(x2, y2)
            glVertex2f(x1, y2)
            glEnd()