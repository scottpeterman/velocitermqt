...
PATCHED: Extended color support (256 color + true color) via SGR override.
"""
...
class FixedHistoryScreen(pyte.HistoryScreen):
    """
    Patched HistoryScreen that:
    1. Handles pyte's CSI dispatch bug (private= kwarg)
    2. Adds 256-color and true color (24-bit) support via SGR override

    Pyte's default SGR handler doesn't parse:
    - 38;5;N (256 foreground)
    - 48;5;N (256 background)
    - 38;2;R;G;B (true color foreground)
    - 48;2;R;G;B (true color background)

    This override intercepts those sequences and stores the color
    as an int (256) or tuple (true color) in char.fg/char.bg.
    """
    ...
    def select_graphic_rendition(self, *attrs, **kwargs):
        ...
        IMPORTANT: Process attrs in order to handle sequences like
        \x1b[0;38;5;51m (reset, then set 256 color) correctly.
        """
        ...
        while i < len(attrs):
            ...
            # Standard attr - let pyte handle it immediately
            # This preserves ordering (e.g., reset before color)
            super().select_graphic_rendition(attr)
            i += 1

    def _set_fg_color(self, color):
        """Set foreground color on cursor attrs."""
        # pyte uses a Char namedtuple for cursor.attrs
        # We need to replace the fg field
        self.cursor.attrs = self.cursor.attrs._replace(fg=color)

    def _set_bg_color(self, color):
        """Set background color on cursor attrs."""
        self.cursor.attrs = self.cursor.attrs._replace(bg=color)

    # ─────────────────────────────────────────────────────────────
    # Other pyte bug fixes (private= kwarg issues)
    # ─────────────────────────────────────────────────────────────
